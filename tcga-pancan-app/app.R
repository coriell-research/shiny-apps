...
sample_types <- c("Primary Solid Tumor", "Solid Tissue Normal", "Metastatic", 
                  "Recurrent Solid Tumor", "Primary Blood Derived Cancer - Peripheral Blood", 
...
col_choices <- data.frame(
  id = c("N/A", "age", "race", "gender", "ajcc_pathologic_tumor_stage", "vital_status", "cancer_type", "os_time", "sample_type"),
  names = c("N/A", "Age", "Race", "Gender", "Pathologic Stage", "Vital Status", "Cancer Type", "Overall Survival Time", "Sample type")
)
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      pickerInput(
        ...
        multiple = TRUE,
        options = pickerOptions(actionsBox = TRUE, size = 6)
      ),
      pickerInput(
        "x_gene",
        label = "Gene (x-axis)",
          ...
          fluidRow(
            column(
              ...
              selectInput(
                "x_col",
                "Color By",
                choices = c("N/A" = "N/A", "Race" = "race", "Gender" = "gender", 
                            "Tumor Stage" = "ajcc_pathologic_tumor_stage",
                            "Vital Status" = "vital_status", "Sample Type" = "sample_type"),
                selected = "N/A"
              )
            ),
...
server <- function(input, output, session) {
    ...
    df <- inner_join(
      x = {
        tbl(pool, input$x_tbl) |>
          ...
          semi_join(y = {
            tbl(pool, "clinical") |> filter(cancer_type %in% !!input$types & sample_type %in% !!input$sample_types)
  ...
  output$data <- render_gt({
    ...
    keep_cols <- c(
      ...
      "ajcc_pathologic_tumor_stage", "vital_status", "cancer_type", "sample_type", "os_time"
    )
    df <- data()[, keep_cols]
