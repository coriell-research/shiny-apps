...
The new format should NOT contain any front matter i.e. only the header followed
...
ssrs_fpath <- here("data", "test-data", "NHGRI Shipping History Detail Q4Y5.csv")
lay_fpath <- here("data", "test-data", "NHGRI_Lay_Summary_Q4Y5_V2_UTF-8.csv")
...
joined <- joinLayOntoSSRS(ssrs_dts, lay_unnested_dt)

joined_ssrs <- joined[["ssrs"]]
joined_lay <- joined[["lay"]]

report_dts <- lapply(joined_ssrs, splitInvestigator)
lay_dts <- lapply(joined_lay, splitInvestigator)
